...
function parseArgs(): Partial<Bun.BuildConfig> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const nextArgIsFlag = args[i + 1]?.startsWith("--");
    const isStandaloneBooleanFlag =
      !hasInlineValue && (isLastArg || nextArgIsFlag);

    if (isStandaloneBooleanFlag) {
    ...
    if (key.includes(".")) {
      ...
      const shouldInitializeParentConfig =
        typeof existing !== "object" ||
        existing === null ||
        Array.isArray(existing);

      if (shouldInitializeParentConfig) {
        config[parentKey] = {};
      }
...
const formatFileSize = (bytes: number): string => {
  ...
  let size = bytes;
  let unitIndex = 0;
  let canScaleToNextUnit = size >= 1024 && unitIndex < units.length - 1;

  while (canScaleToNextUnit) {
    size /= 1024;
    unitIndex++;
    canScaleToNextUnit = size >= 1024 && unitIndex < units.length - 1;
  }
...
const outdir = cliConfig.outdir ?? defaultOutdir;
